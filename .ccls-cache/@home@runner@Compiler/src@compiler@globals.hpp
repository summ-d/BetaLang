...
typedef struct IOError : public virtual err::BaseError {
  ...
  void print() const override {
    std::cout << "error Opening File:" << this->raw << "info:\n"
              << "Line:" << this->line << '\n'
              << "Message:" << this->rawTwo << '\n';
  }
...
typedef struct UnterminatedExpressionError : public virtual err::BaseError {
  void print() const override {
    std::cout << "Error on parsing file:" << this->line << "Info:\n"
...
util::RelationalMap<bdis_t, long> parseBasic(util::stringlist lines){
...
private:
  const char *rawOne = qualifier.asCstr();
  const char *rawTwo = message.asCstr();
...
util::string posQual[10] = {"async",     "static",   "const",    "volatile",
                            "constexpr", "restrict", "register", "pub",
                            "priv",      "prop"};

Type getType(util::string str) {
  int i;
  for (i = 0; i < 23; i++)
    if (util::strcmp(str.asCstr(), possTypes[i].asCstr()))
      break;
  switch (i) {
  case 0:
    return Type::INTEGER;

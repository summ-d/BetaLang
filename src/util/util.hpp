...
class AbstractSyntax: public CollectionsBase<AbstractType>{
  ...
  public:
  AbstractSyntax(LinkedList<AbstractType>& linkedList){

  }

  void getFirstLevelTokens(AbstractType arr[], int count){
    firstLevel = new LinkedList<AbstractType>(arr, count);
  }

  void getSecondLevelTokens(AbstractType arr[], int count){
    secondLevel = new LinkedList<AbstractType>(arr, count);
  }
